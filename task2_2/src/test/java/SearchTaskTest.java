...
public class SearchTaskTest {
    ...
    @Test
    public void searchShouldFindExistingTask(){
        ...
        mainPage.searchTask(TASK_NAME);
        assertTrue(mainPage.allFoundTasksContain(TASK_NAME));
    }

    @Test
    public void searchDoesntExistTaskShouldReturnEmptyList(){
        final String DOESNT_EXIST_TASK_NAME = FieldGenerator.getTaskName();
        driver = WebDriverFactory.createDriver(BrowserType.CHROME);
        MainPage mainPage = new MainPage(driver);
        mainPage.open();
        mainPage.searchTask(DOESNT_EXIST_TASK_NAME);
        assertTrue(mainPage.checkNoOneTasksIsExist());
    }
}
